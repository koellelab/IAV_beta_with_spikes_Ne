...
swine_flumina_alt <- swine_flumina_alt %>%
  ...
  mutate(Subset = if_else(freq1 >= threshold & gen > 0 & row_number() == which.min(abs(freq1 - 0.5)), 
                          1, Subset)) %>% # assign subsets 1
  ungroup()

# export table S3
table_S3 <- swine_flumina_alt[, c("ID", "iSNV", "DPS1", "DPS2", "freq1", "freq2", "Subset")]
colnames(table_S3) <- c("Individual ID", "iSNV", "DPS1", "DPS2", "DPS1 Frequency", "DPS2 Frequency", "Subset")

write.csv(table_S3, "03_data/table_S3.csv", row.names = FALSE, quote = FALSE)
