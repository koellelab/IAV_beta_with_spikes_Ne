...
set.seed(27)
threshold <- 0.005
flumina_0.005_origin <- read_csv("03_data/processed_vcf_wide_minAF0.005.csv")[, c(1:6, 28:29, 31, 33:37)]
flumina_0.005_origin$index <- 1:nrow(flumina_0.005_origin)

dps_values <- c(-2, 0:6)

McCrone_IAV_flumina_data <- flumina_0.005_origin %>%
  filter(data == "mccrone") %>%
  rowwise() %>%
...
write.csv(table_S1, "03_data/table_S1_05.csv", row.names = FALSE, quote = FALSE)
