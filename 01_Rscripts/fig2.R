...
McCrone_tot_obs[McCrone_tot_obs$freq == 0, "type"] <- "observation 2 below 2%"
McCrone_tot_obs[McCrone_tot_obs$freq == 0, "freq"] <- 0.02
...
list_ll_max <- 1:length(list_df_sim_bs) %>%
  mclapply(function(x) {
    ...
    mock_ll_df <- LL_Ne.df(list_df_sim_bs[[x]], seq(10, 100, 1), mu, alpha, hourPerGen, m)
...
fig2b <- ggplot(LLdf_McCrone) +
  ...
  annotate("text", x = 70, y = -38, label = paste("N[E] == ", MLE_Ne_McCrone), parse = TRUE, color = "red") +
  ...
  scale_x_continuous(breaks = seq(0, 100, 20), limits = c(0, 101))
...
ggsave(fig2, file = "fig2_log.pdf", path = "02_plots", width = 10, height = 8)
